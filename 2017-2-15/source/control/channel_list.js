...
function getData(){
	...
	var _categoryID = base.getParmar("categoryID");

	baseAjax.getCategoryById(_categoryID,function(data){
		var $target = $(".channel-list-data-area");
		var _html = "";
		if(data) {
			...
			if(_data.length == 1){
				$.mobile.changePage("text_list.html?channelID=" + _data[0].channelID+"&channelType="+ _data[0].channelType +"&channelName="+ _data[0].channelName, {
					transition: "pop",
					reverse: false,
					changeHash: false
				});
			...
			for(var i=0; i<_data.length; i++){
				var _this = _data[i];
				var _url = "text_list.html?channelID=" + _this.channelID+"&channelType="+ _this.channelType +"&channelName="+ _this.channelName;
